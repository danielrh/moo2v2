...
---
Optional improvements (improvements.md) — done after the bug list:

- [x] Target most-damage ship without overkill — new "deadliest" target priority (highest sustained weapon output first), and the overkill switch now triggers at ~80% projected kill instead of only at 100%, so focused fire stays valuable but a wall of damage-soakers can't hold a volley hostage — the extra shots walk to the next live target. Deterministic; tested. The AI's militarist/rusher profiles use it.
- [x] Standoff no longer just pushes off the edge — when pinned against a field boundary it strafes ALONG the edge instead of grinding into the clamp.
- [x] Planet takeovers / transports / ground animation — ground invasions record round-by-round troop counts and emit a replay event to the two PARTICIPANTS ONLY (never spectators); the Empires tab lists "Ground assaults" with a ▶ watch button that opens an animated GroundBattleDialog (shrinking troop rows, scrubber, verdict). Tested (capture, repel, participant-only visibility).
- [x] Bot personalities — the fair bot now has five distinct, tuned play styles (techer / rusher / industrialist / expander / militarist) plus balanced, varying research emphasis, fleet ratio, expansion depth, buying, and aggression/target priority. Selectable on the home screen (or "random style"); subbed-in bots auto-vary by seat. A round-robin self-play test proves each personality is viable (survives, stays ≥60% of the balanced baseline's score) and genuinely different (techer out-researches the militarist, the militarist out-builds warships, the expander holds the most colonies). v2-beats-v1 self-play still holds.
- [x] Large-empire start option — lobby "Big empires start": each player begins with a bubble of 10-20 colonies around the homeworld, each 1/3-1/2 populated, assigned by nearest-home so the territories are contiguous and non-overlapping and never seize a monster-guarded system. Tested across seeds.

Verification (2026-07-10): `npm test` 338 passed (56 files; added groundbattle, bigstart, personalities, selfplay, combatfixes deadliest), `npm run test:game` 10 passed, `MOO2_BALANCE=1` envelope (combat perf regression from the overkill ledger fixed — folded once per tick, not per shot), `svelte-check` 0 errors, `npm run build`, Playwright 8/8. ENGINE_VERSION 0.6.0 → 0.7.0; golden fixture confirms old saves load snapshot-first.
